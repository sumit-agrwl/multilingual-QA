...
class MTLDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        target = self.get_target(examples[0])

        if 'ctxs' in examples[0] and self.n_context is not None:
            f = self.title_prefix + " {} " + self.passage_prefix + " {}"
            contexts = examples[0]['ctxs'][:self.n_context]
            passages = [f.format(c['title'], c['text']) for c in contexts]
            scores = [float(c['score']) for c in contexts]
            scores = torch.tensor(scores)
        else:
...
class MTLCollator(object):
    ...
    def __call__(self, batch):
        ...
        text_passages_0 = [[example['questions'][0] + " " + t for t in example['passages']] for example in batch]
        text_passages_1 = [[example['questions'][1] + " " + t for t in example['passages']] for example in batch]
        passage_ids_0, passage_masks_0 = encode_passages(text_passages_0,
                                                     self.tokenizer,
                                                     self.text_maxlength)
        passage_ids_1, passage_masks_1 = encode_passages(text_passages_1,
                                                     self.tokenizer,
                                                     self.text_maxlength)

        return (index, target_ids, target_mask, passage_ids_0, passage_masks_0, passage_ids_1, passage_masks_1)
...
def load_mtl_data(data_path=None, global_rank=-1, world_size=-1):
    assert data_path
    if data_path.endswith('.jsonl'):
        data = open(data_path, 'r')
    elif data_path.endswith('.json'):
